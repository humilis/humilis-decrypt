...
HUMILIS_ENV := humilis-decrypt
HUMILIS_DB_ENV := humilis-db
...
db: develop
	$(HUMILIS) update \
		--stage $(STAGE) \
		--output $(HUMILIS_DB_ENV)-$(STAGE).outputs.yaml \
		$(HUMILIS_ENV_PATH)$(HUMILIS_DB_ENV).yaml.j2
...
delete: develop
	...
	$(HUMILIS) delete --stage $(STAGE) $(HUMILIS_ENV_PATH)$(HUMILIS_DB_ENV).yaml.j2
